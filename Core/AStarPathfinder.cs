...
namespace AStarShowcase.Core
{
    public class PathResult<TNode> where TNode : INode
    {
        ...
        public PathResult(bool success, List<TNode> path, IReadOnlyCollection<TNode> exploredNodes, int nodesEvaluated, double elapsedMilliseconds)
        {
            Success = success;
            Path = path;
    ...
    public class AStarPathfinder<TNode> where TNode : IHasXmlNode
    {
        ...
        public PathResult<TNode> FindPath(TNode start, TNode goal)
        {
            ...
            {
                TNode current = openSet.ExtraMin();
                nodesEvaluated++;
                if (Equals(current, goal))
                {
                    stopwatch.Stop();
                    var path = ReconstructionPAth(cameFrom, current);
                ...
                foreach (TNode neighbor in _grid.GetNeighbors(current))
                {
                    ...
                    float tenmtativeGScore = gScore[current] + _grid.GetMovementCost(current, neighbor);
                    bool neighborIsNew = !gScore.ContainsKey(neighbor);
                    if (neighborIsNew || tenmtativeGScore < gScore[neighbor])
                    {
                        cameFrom[neighbor] = current;
                        gScore[neighbor] = tentativeGScore;
                        fScore[neighbor] = tentativeGScore + _grid.GetHeuristic(neighbor, goal);
                        if (neighborIsNew) openSet.Insert(neighbor);
                        else openSet.UpdatePriority(neighbor);
                    }
                }
            }
            stopwatch.Stop();
